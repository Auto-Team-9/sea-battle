...
const Button = ({ variant = 'primary', icon, className = '', children, ...props }: ButtonProps) => {
  return (
    <button
      className={`flex w-full cursor-pointer items-center justify-center gap-2 rounded-md border px-3 py-1.5 text-sm font-semibold transition duration-300 ease-in-out focus-visible:outline-2 active:scale-95 ${variantStyles[variant]} ${className}`}
      {...props}
    >
      {variant === 'arrow' ? (
